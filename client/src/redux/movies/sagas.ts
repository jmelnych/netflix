...
export function* fetchMovies() {
  try {
    const result = yield call(api.fetchMovies);
    yield put(actions.fetchMoviesSuccess(result));
    yield put(actions.setGenres(result));
  } catch (e) {
    console.error(e);
...
export default [
  takeEvery(types.FETCH_MOVIES, fetchMovies),
  takeEvery(types.FETCH_MOVIE, fetchMovie),
  takeEvery(types.UPDATE_QUERY, watchQueryUpdate),
  takeEvery(types.EDIT_MOVIE, editMovie),
  takeEvery(types.DELETE_MOVIE, deleteMovie),
  takeEvery(types.CREATE_MOVIE, createMovie),
];
